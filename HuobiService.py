...
Market data API
'''
def get_token(yanzhengma):
    """
    :param yanzhengma
    :return:
    """
    params = {'email_code': yanzhengma,
              'use_type': "VERIFY_SETTING_POLICY"}

    url = MARKET_URL + '/-/x/uc/uc/open/security/strategy/verify'
    return http_get_request(url, params)

def login():
    params = {'fingerprint': "cccf6b5e8118a0235ac777e20f07f9f8",
              'login_name': "wflin2018@163.com",
              'password': "770bb5b0ea182d488143761f9041e5a1",
              'way': "WEB"}

    url =  MARKET_URL + "/-/x/uc/uc/open/login"
    #登录
    userLogin = http_post_request(url, params);
    #发送验证码
    userSend = send();
    #token
    userToken=get_token();
    #发送订单
    userOrder=send_ponter_transfer_order()
    return userOrder;

def send():
    params = {'use_type': "VERIFY_SETTING_POLICY"}

    url = MARKET_URL + '/-/x/uc/uc/open/email_code/send'
    msg = api_key_post(params, url)
    return msg

# 点卡转让
def send_ponter_transfer_order(yanzhengma,currency, face_account, face_uid, points, price=0):
    """
    :param currency:
    :param source: 'web'
    :param face-accoun:
    ...
    url = '/v1/points/transfer/create?auth-token='
